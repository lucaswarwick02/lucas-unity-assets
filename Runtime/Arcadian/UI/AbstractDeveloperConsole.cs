...
namespace Arcadian.UI
{
    ...
    public abstract class AbstractDeveloperConsole : MonoBehaviour
    {
        ...
        {
            var key = ArcadianAssetsSettings.GetOrCreate().developerConsoleKey;
            var type = ArcadianAssetsSettings.GetOrCreate().developerConsoleType;

            if (type.Type == null)
            {
                Debug.LogWarning("[Arcadian] Developer Console 'type' not set. Please set the 'type' in the Arcadian Assets project settings.");
                return;
            }

            if (key == Key.None)
            {
                Debug.LogWarning("[Arcadian] Developer Console 'key' not set. Please set the 'key' in the Arcadian Assets project settings.");
                return;
            }
            ...
            (new GameObject { name = "[Arcadian] Developer Console" }.AddComponent(type) as AbstractDeveloperConsole).Key = key;
        }
    }
